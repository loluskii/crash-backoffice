                    ...
                    <tbody>
                        <tr v-for="(result, index) in results" v-if="results.length > 0 && !loading" :key="index">
                          <td>{{ formatDate(result.created_at, 'DD/MM/YYYY HH:mm:ss') }}</td>
                            ...
                            <td>{{ formatNumber(parseFloat(result.from_user_balance) + parseFloat(result.amount)) }}</td>
                            <td>{{ formatNumber(result.from_user_balance) }}</td>
                        </tr>
...
import Reports from '../../services/reports'
    ...
    export default {
        ...
        methods:{
            ...
            getResults(page){
                this.loading = true
                Reports.getSystemTransactions(this.filterData, page).then(res=>{
                    this.loading = false;
        ...
        asyncData ({ store }) {
            store.dispatch('setTitle', 'System Transactions')
        },
        mounted(){
