...
export default {
  ...
  methods:{
    ...
    getSales(page){
      this.loading = true
      Bets.getPoolSales(this.filterData, page).then(res=>{
        this.loading = false;
    ...
    getTickets(page, shopId){
      ...
      Bets.getPoolTickets(this.filterData, page, shopId).then(res=>{
  ...
  asyncData ({ store }) {
    store.dispatch('setTitle', 'Pools Sales Report')
  },
  mounted(){
