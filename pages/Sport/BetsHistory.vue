    ...
    <div>
        <form id="single-stop" @submit.prevent="getResults(1)">
            ...
            <div class="row m-t-10">
                ...
                <br class="d-sm-none">
                <div class="col-sm-2">
                    <select name="class" id="status" class="form-control" v-model="filterData.paid_status">
            ...
            <div class="table-responsive" >
                <table class="table table-bordered table-striped">
                    ...
                    <tr>
                        ...
                        <td>{{ bet.bet_type_desc}} ({{ bet.event_type === 'pre' ? 'P' : 'L' }})</td>
                        ...
                        <td>{{ bet.events}}</td>
                        <td>{{ bet.markets}}</td>
                        <td>{{ bet.lost_events_count}} Events</td>

                        <td>{{ bet.channel }}</td>
                        <td>{{ formatNumber(bet) }}</td>
                        <!-- <td>100.00%</td> -->
                        ...
                        <button type="button" v-permission="'Mark as won'"
                          ...
                          style="color: #757575" v-if="bet.status !== 1 && bet.status !== 5" @click="updateTicket(bet.id, 'won')">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getResults(page){
                this.loading = true
                Bets.getBetHistory(this.filterData, page).then(res=>{
                    this.loading = false;
            ...
            calcWinLoss(bet){
                if(bet.status === 1 || bet.status === 5){
                    if (parseFloat(bet.winnings) < parseFloat(bet.stake)) {
                      return "+" + this.formatNumber((parseFloat(bet.stake) - parseFloat(bet.winnings)), true);
                    } else {
                      return "-" + this.formatNumber((parseFloat(bet.winnings) - parseFloat(bet.stake)), true);
                    }
        ...
        asyncData ({ store }) {
            store.dispatch('setTitle', 'Bet History')
        },
        mounted(){
