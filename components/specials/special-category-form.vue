<template>
    <form class="form-horizontal columns column is-multiline is-12" id="cat-form" role="form" @submit.prevent="saveCategory">
        <div class="row form-group">
            ...
            <div class="col-md-4">
            <input type="hidden" value="" name="category_id" v-model="formData.id">                
                <button class="button btn btn-primary mr-2" type="submit" id="saveCategory" name="button"><i class="fa fa-check"></i></button>
                <button class="button btn btn-secondary" type="button" name="button" @click="resetForm"><i class="fa fa-refresh"></i></button>
            </div>
        </div>
            ...
            <tr v-for="(category, index) in categories" :key="category.id">
                ...
                <td>
                    <div class="btn-group btn-sm">
                        <button type="button" class="btn btn-sm btn-info" @click="edit(category)"><i class="fa fa-pencil"></i></button>
                        <button type="button" class="btn btn-sm btn-danger" @click="deleteCat(category.id, index)"><i class="fa fa-trash-o"></i></button>
                    </div>
                </td>
            </tr>
        </table>
        ...
        methods:{
            ...
            async saveCategory(){
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        ...
                        CMS.saveSpecialCategory(this.formData).then( res => {
            ...
            deleteCat(id, i){
                ...
                self.$swal({
                    ...
                    if (result.value) {
                        CMS.deleteSpecialCategory(id).then((res) => {
                            if(res.success){
                                // $(e.target).closest('tr').remove();
                                this.$nuxt.$emit('CATEGORY_REMOVED', i); 
                                this.notify({message: 'Item has been removed', title: 'Deleted!', type: 'success'});
                            }else{
